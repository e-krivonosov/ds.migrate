...
use Exception;
use Ds\Migrate\Builders\AgentBuilder;
use Ds\Migrate\Builders\BlankBuilder;
use Ds\Migrate\Builders\CacheCleanerBuilder;
use Ds\Migrate\Builders\EventBuilder;
use Ds\Migrate\Builders\FormBuilder;
use Ds\Migrate\Builders\HlblockBuilder;
use Ds\Migrate\Builders\HlblockElementsBuilder;
use Ds\Migrate\Builders\IblockBuilder;
use Ds\Migrate\Builders\IblockElementsBuilder;
use Ds\Migrate\Builders\MarkerBuilder;
use Ds\Migrate\Builders\OptionBuilder;
use Ds\Migrate\Builders\TransferBuilder;
use Ds\Migrate\Builders\UserGroupBuilder;
use Ds\Migrate\Builders\UserOptionsBuilder;
use Ds\Migrate\Builders\UserTypeEntitiesBuilder;
use Ds\Migrate\Schema\AgentSchema;
use Ds\Migrate\Schema\EventSchema;
use Ds\Migrate\Schema\GroupSchema;
use Ds\Migrate\Schema\HlblockSchema;
use Ds\Migrate\Schema\IblockSchema;
use Ds\Migrate\Schema\OptionSchema;
use Ds\Migrate\Schema\UserTypeEntitiesSchema;
...
class VersionConfig
{
    ...
    public function __construct($configName = '')
    {
        ...
        if (!isset($this->configList['archive'])) {
            $this->configList['archive'] = $this->prepare('archive', [
                ...
                'migration_table' => 'ds_migrate_archive',
            ]);
        }
    ...
    protected function prepareValues($values = [])
    {
        ...
        if (empty($values['migration_table'])) {
            $values['migration_table'] = 'ds_migrate_versions';
        }
    ...
    public function createConfig($configName, $configValues = [])
    {
        ...
        if (isset($this->configList[$configName])) {
            ...
            $configDefaults = [
                ...
                'migration_table' => 'ds_migrate_' . $configName,
            ];
        }
